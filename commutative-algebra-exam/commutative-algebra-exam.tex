...
$$
\varphi:(t,s) \longmapsto (t^2,t s,s^2)
$$
...
$$
To check this we need to prove that its 
kernel is indeed $(y^2-xz)$.
For this let $F \in \Ker \varphi$.
Consider $F$ as a polynomial in $y$
and use division algorithm by
$y^2-xz$ to obtain:
$$
F=Q(y^2-xz)+A(x,z)+yB(x,z).
$$
since the residue must have degree in $y$
strictly less than
$2=\operatorname{deg}(y^2-xz)$. Applying
$\varphi$ we obtain
$$
0=A(s^2,t^2)+stB(s^2,t^2),
$$
which have different parity degrees
on $s$ and $t$, meaning both summands
are zero. The fact that $(y^2-xz) \subset
\Ker\varphi$ is obvious, so we have equality
as desired.
