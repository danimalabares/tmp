...
Integration by parts in $x$ gives a factor $in$. Periodicity cancels
the boundary term. Multiply by $e^{-imy}$, integrate in $y$, and normalize:
\[
 (g_x)_{n,m}=\frac1{(2\pi)^2}\int_E g_x e^{-i(nx+my)}\,dx\,dy
 =in\,g_{n,m}.
\]
Here $(g_x)_{n,m}$ means the Fourier coefficient of $g_x$.
Apply the same identity to $g_x$, then to $g_{xx}$ and $g_{xxx}$:
\[
 \begin{aligned}
 (g_{xx})_{n,m}&=in\,(g_x)_{n,m}=(in)^2g_{n,m},\\
