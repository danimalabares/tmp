...
The image \emph{sheaf} consists of sections that are locally images.
A single lift on the original open set is an additional requirement.
\end{frame}
...
\]
For $\F=\Omega^p_M$, we have two resolutions:
\[
 \begin{aligned}
 ...
 \end{aligned}
\]
Injective sheaves are acyclic. Their resolution defines sheaf cohomology.
The fine Dolbeault resolution computes the same groups:
\[
 \boxed{
 H^q(M,\Omega^p_M)
 \cong H^q\bigl(\Gamma(M,\A^{p,\bullet}_M)\bigr)
 =H^{p,q}_{\db}(M).
 }
\]
The comparison concerns cohomology \textbf{after taking global sections}.
