...
\begin{frame}{From de Rham to Dolbeault}
The \texttt{de-rham-dolbeault} notes use the same method for both theorems:
compute sheaf cohomology using an \textbf{acyclic resolution}.

\textbf{de Rham (Voisin, Corollary 4.37).}
\[
 H^q(M,\R_M)\cong H^q_{\mathrm{dR}}(M).
\]
The resolution is the complex of smooth differential forms with $d$.

\textbf{Dolbeault.} On a complex manifold $M$,
\[
 H^q(M,\Omega^p_M)\cong H^{p,q}_{\db}(M).
\]
Here $\Omega^p_M$ is the sheaf of holomorphic $p$-forms.
We use smooth $(p,q)$-forms with $\db$ instead.

The common tool is \textbf{Voisin's Proposition 4.32}.
\src{Voisin, \emph{Hodge Theory and Complex Algebraic Geometry I}, Chapter 4;\newline
\texttt{de-rham-dolbeault} notes, de Rham's theorem and Dolbeault's theorem.}
\end{frame}

\begin{frame}{The key tool: Voisin, Proposition 4.32}
\textbf{Acyclicity (Definition 4.31).}
An object $B$ is acyclic for a left-exact functor $F$ if
\[
 R^rF(B)=0\qquad\text{for all }r>0.
\]
\textbf{Proposition 4.32.}
Let $0\to A\to B^0\to B^1\to\cdots$ be a resolution of $A$.
If \textbf{every $B^j$ is acyclic for $F$}, then
\[
 \boxed{R^qF(A)\cong H^q\bigl(F(B^\bullet)\bigr).}
\]
For sheaves, take $F=\Gamma(M,-)$, so $R^qF(A)=H^q(M,A)$.

We need \textbf{both} an exact resolution and acyclic terms to compute
sheaf cohomology from its complex of global sections.
\src{Voisin, \emph{Hodge Theory and Complex Algebraic Geometry I},\newline
Definition 4.31 and Proposition 4.32; $\S$4.3 for sheaf cohomology.}
\end{frame}

\begin{frame}{The acyclicity hypothesis}
Write $\A^{p,q}_M$ for the sheaf of smooth $(p,q)$-forms.

\textbf{Fine sheaves (Voisin, Definition 4.35).}
Sheaves of $C^\infty_M$-modules are fine: multiplication by a smooth
partition of unity supplies a partition of unity on the sheaf.
In particular, each $\A^{p,q}_M$ is fine.

\textbf{Voisin, Proposition 4.36.}
A fine sheaf $\mathcal B$ is acyclic for global sections:
\[
 H^r(M,\mathcal B)=0\qquad(r>0).
\]
Thus all the terms $\A^{p,q}_M$ satisfy the \textbf{acyclicity hypothesis}
of Proposition 4.32.

We still need the Dolbeault complex to be a \emph{resolution}.
\src{Voisin, \emph{Hodge Theory and Complex Algebraic Geometry I},\newline
Definition 4.35 and Proposition 4.36, as in the \texttt{de-rham-dolbeault} notes.}
\end{frame}

\begin{frame}{Why we need the $\bar\partial$-Poincar\'e lemma}
Consider the sheaf complex
\[
 0\longrightarrow\Omega^p_M\longrightarrow\A^{p,0}_M
 \xrightarrow{\db}\A^{p,1}_M\xrightarrow{\db}\cdots.
\]
At degree zero, the kernel consists of holomorphic $p$-forms.
At positive degree, the $\db$-Poincar\'e lemma supplies local primitives:
\[
 q>0,\quad\db\eta=0\quad\Longrightarrow\quad\eta=\db\beta
 \quad\text{locally}.
\]
This makes the sheaf complex \textbf{exact}.
Its terms are \textbf{acyclic} by Proposition 4.36.

We can therefore apply \textbf{Proposition 4.32}:
\[
 H^q(M,\Omega^p_M)
 \cong H^q\bigl(\A^{p,\bullet}(M),\db\bigr)
 =H^{p,q}_{\db}(M).
\]
Today's task is to establish the local exactness in one variable.
\src{Voisin, \emph{Hodge Theory and Complex Algebraic Geometry I}, Chapter 4,\newline
Propositions 4.32 and 4.36; \texttt{de-rham-dolbeault}, Dolbeault's theorem.}
\end{frame}
...
\]
Every $(0,1)$-form is $\db$-closed: $d\bar z\wedge d\bar z=0$.
...
\begin{frame}{Why an orthonormal basis gives a series}
\textbf{Theorem.} For a countable orthonormal basis $(e_j)$,
\[
 v=\sum_j\langle v,e_j\rangle e_j,
 \qquad \|v\|^2=\sum_j|\langle v,e_j\rangle|^2.
\]
\textbf{Proof.} Write $S_N=\sum_{j=1}^N\langle v,e_j\rangle e_j$.
Orthogonality gives
\[
 \|v-S_N\|^2=\|v\|^2-\sum_{j=1}^N|\langle v,e_j\rangle|^2\geq0.
\]
The numerical sum converges, so $(S_N)$ is Cauchy and has a limit $s$.
For every $j$, $\langle v-s,e_j\rangle=0$.
Density then gives $v=s$. The norm identity follows.\hfill$\square$

For $L^2$, this is convergence in norm; pointwise convergence is a
separate question.
\end{frame}
...
\textbf{Completeness.} Finite sums of $e^{int}$ are trigonometric
polynomials. They form a subalgebra of $C(S^1,\C)$, contain constants,
separate points, and are closed under complex conjugation.

The complex Stone--Weierstrass theorem therefore makes them uniformly
dense in $C(S^1,\C)$. Continuous functions are dense in $L^2(S^1)$,
and $\|h\|_2\leq\sqrt{2\pi}\|h\|_\infty$.
Thus their span is dense in $L^2$.\hfill$\square$
\src{This is the completeness argument in Verbitsky, Lecture 12, pp. 2--4.}
\end{frame}
...
Chapter 4: Definitions 4.31, 4.35; Propositions 4.32, 4.36;
Corollary 4.37.
...
\item Daniel Gonz\'alez Casanova Azuela,
\href{https://github.com/danimalabares/tmp/blob/main/de-rham-dolbeault/de-rham-dolbeault.tex}{\texttt{de-rham-dolbeault} notes}: acyclic resolutions,
fine sheaves, and the proof of Dolbeault's theorem.
\item Daniel Gonz\'alez Casanova Azuela,
\href{https://github.com/danimalabares/functional-analysis/blob/main/functional-analysis/functional-analysis.tex}{\texttt{functional-analysis/functional-analysis.tex}}:
``Orthogonal projections'', the examples beginning
``the collection (or any subset thereof)''.
The quoted text and formulas are reproduced verbatim.
\end{itemize}
\end{frame}
\end{document}
