...
\begin{frame}{Lecture 11: Dolbeault's theorem}
Let $M$ be a complex manifold, $\Omega^p_M$ the sheaf of holomorphic
$p$-forms, and $\A^{p,q}_M$ the sheaf of smooth $(p,q)$-forms.

\textbf{Theorem.}
\[
 H^q(M,\Omega^p_M)\cong H^{p,q}_{\db}(M)
 =\frac{\ker(\db:\A^{p,q}(M)\to\A^{p,q+1}(M))}
 {\operatorname{im}(\db:\A^{p,q-1}(M)\to\A^{p,q}(M))}.
\]
\textbf{Method.} Use the acyclic resolution
\[
0\longrightarrow\Omega^p_M\longrightarrow\A^{p,0}_M
 \xrightarrow{\db}\A^{p,1}_M\xrightarrow{\db}\cdots.
\]
Exactness comes from the $\db$-Poincar\'e lemma.
Acyclicity comes from partitions of unity.
\src{Verbitsky, Lecture 11, pp. 20, 28; \texttt{de-rham-dolbeault} notes, Dolbeault's theorem.}
\end{frame}

\begin{frame}{Why an acyclic resolution computes cohomology}
\textbf{Recall.} A sheaf $\mathcal B$ is acyclic if
$H^r(M,\mathcal B)=0$ for $r>0$.

\textbf{Theorem.} If $0\to\F\to\mathcal B^0\to\mathcal B^1\to\cdots$
is exact and each $\mathcal B^j$ is acyclic, then
\[
 H^q(M,\F)\cong H^q\bigl(\Gamma(M,\mathcal B^\bullet)\bigr).
\]
\textbf{Proof.} Put $Z^0=\F$ and
$Z^j=\ker(\mathcal B^j\to\mathcal B^{j+1})$.
The sequences $0\to Z^j\to\mathcal B^j\to Z^{j+1}\to0$ give
\[
 \frac{\Gamma(Z^q)}{\operatorname{im}\Gamma(\mathcal B^{q-1})}
 \cong H^1(M,Z^{q-1})\cong\cdots\cong H^q(M,\F)
 \qquad(q\geq1).
\]
These are the long exact cohomology sequences, using acyclicity.
For $q=0$, the kernel is $\Gamma(M,\F)$.\hfill$\square$
\src{Verbitsky, Lecture 11, pp. 18, 20, 27; \texttt{de-rham-dolbeault}, Proposition 4.32.}
\end{frame}

\begin{frame}{The Dolbeault resolution: exactness}
\textbf{At degree zero.} In holomorphic coordinates,
\[
 \alpha=\sum_{|J|=p} f_J\,dz_J,
 \qquad \db\alpha=0
 \quad\Longleftrightarrow\quad
 \frac{\partial f_J}{\partial\bar z_k}=0\quad\text{for all }J,k.
\]
Thus $\ker(\db:\A^{p,0}_M\to\A^{p,1}_M)=\Omega^p_M$.

\textbf{At positive degree.} The $\db$-Poincar\'e lemma says that
if $q>0$ and $\db\alpha=0$, then locally $\alpha=\db\beta$.

Exactness of sheaves is exactness on stalks, so local primitives suffice.

This is the input we begin proving today.
\src{Verbitsky, Lecture 11, pp. 8, 28; \texttt{de-rham-dolbeault}, Dolbeault proof, steps 1--2.}
\end{frame}

\begin{frame}{The Dolbeault resolution: acyclicity}
Each $\A^{p,q}_M$ is a sheaf of $C^\infty_M$-modules, hence fine.
Here is why partitions of unity kill its higher cohomology.

Let $(\rho_j)$ be a subordinate partition of unity, and let
$c=(c_{i_0\ldots i_r})$ be an alternating \v{C}ech cocycle, $r>0$.
Define
\[
 b_{i_0\ldots i_{r-1}}=\sum_j\rho_j c_{j i_0\ldots i_{r-1}}.
\]
Products extend by zero, and the sum is locally finite. Since $\delta c=0$,
\[
 (\delta b)_{i_0\ldots i_r}
 =\sum_j\rho_j\sum_{k=0}^r(-1)^k c_{j i_0\ldots\widehat{i_k}\ldots i_r}
 =\sum_j\rho_j c_{i_0\ldots i_r}=c_{i_0\ldots i_r}.
\]
Thus $H^r(M,\A^{p,q}_M)=0$ for $r>0$.
The acyclic resolution theorem now gives Dolbeault's theorem.\hfill$\square$
\src{Verbitsky, Lecture 11, pp. 11--18; \texttt{de-rham-dolbeault}, Propositions 4.36 and 4.32.}
\end{frame}
